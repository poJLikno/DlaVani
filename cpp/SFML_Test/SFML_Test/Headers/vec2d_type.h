...
public:
    ...
    T get_radians() {
        T velocity_angle = (T)asin(get_y() / get_length());
        if (get_x() / get_length() < (T)0.0) velocity_angle = (T)3.1415 - velocity_angle;
        if (velocity_angle < (T)0.0) velocity_angle = (T)2.0 * (T)3.1415 + velocity_angle;
